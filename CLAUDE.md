...
- **同步狀態可見(T19)**:`AppController.syncMemosWithAll` 不再全吞例外——同群組且可連線的裝置**全部**失敗時 `memoSyncFailing=true` 並記 `lastMemoSyncAt`(任一成功即更新、清除 failing);沒有可同步對象時維持現狀不算失敗。只在 failing「有/無」跨越時 `notifyListeners`(不每 15 秒重繪)。備忘錄頁 AppBar 在 `memoSyncFailing || clockSkew!=null` 時顯示琥珀色 `sync_problem` 鈕,點開說明對話框(上次成功時間相對顯示)。改群組碼後若 `memoSyncFailing` 跳 SnackBar。
- **手動匯出/匯入(T20)**:備忘錄頁三點選單「匯出備忘錄」(`_exportMemos`)/「匯入備忘錄」(`_importMemos`)。用既有相依 **`file_picker` 11.0.2 靜態方法**(`FilePicker.saveFile`/`FilePicker.pickFiles`,非 `.platform`);匯出檔名 `syncnest_memos_YYYYMMDD_HHmm.json`,桌面回傳路徑後自行 `writeAsString`、行動裝置帶 `bytes` 由套件寫檔。匯入走 `MemoStore.importJson`:與 `mergeJson` 同樣 LWW 合併(不覆蓋較新本機資料),但屬使用者主動本地操作故走 `_commit()`→觸發 `onLocalChange` 推送同步(mergeJson 則刻意不推)。回傳套用筆數(-1=格式錯)。
- **LWW 時鐘偏移防護(T21)**:(1)**本機時間戳單調遞增**:`MemoStore._monotonicNow()`=`max(now, 本機最大 updatedAt+1)`,所有本地變更改走 `_touch(memo)`(取代 `Memo.touch()`)、`add()` 也套用,防本機時鐘曾調快又調回時產生「比自己既有資料還舊」的時間戳而自我覆蓋。(2)**跨裝置偏移偵測**:同步請求/回應互帶 header `x-sync-time`(epoch ms,向後相容,舊版不帶即略過);傳送端 `syncMemos` 用送出/收到中點抵銷來回延遲後算 `對端-本機`、接收端 `memos/sync` route 直接比對,經 `Transport.onClockSkew`(抽象 setter)回呼 `AppController._onClockSkew`,超過 `_clockSkewThreshold`(2 分鐘)才記 `clockSkew` 並在跨門檻時 notify。**不動同步 body 協定核心**。
...
- **Windows 背景到點替代提醒(T16,僅本分支)**:Windows 無背景排程,App 若還在執行但視窗縮到系統匣/最小化,`_fireAlarm`(`alarm_page.dart`)到點時除了 `showNow` 通知氣泡 + `alarm.start()` 響鈴,再 `DesktopTrayService.instance?.bringToForeground()`(叫回並聚焦視窗)。判斷用 `!notifications.supportsScheduling`(涵蓋 Windows),`bringToForeground` 內再 guard `isDesktop`。`DesktopTrayService.instance` 由建構子設(main 的全域 `desktopTray`)。**首次按「啟動」**(`_handleStart`)在 Windows 跳一次性說明對話框(旗標 appSupport `alarm_win_notice_shown`,只顯示一次),頁面底部另有常駐提示「需保持 App 執行才會到點提醒」。
...
- **`objectVersion` 每次 build 被改回 54 的真相與根治**:元凶是 **Flutter 的 `XcodeProjectObjectVersionMigration`**(`flutter_tools/lib/src/migrations/xcode_project_object_version_migration.dart`),每次 `flutter build ios`/`flutter run` **無條件**把 `objectVersion = \d+;` 硬改成 `54`(連同 `LastUpgradeCheck=1510`),**無開關可關**;純 Xcode GUI build 不觸發(不跑 flutter migrator)。本專案有 `PBXFileSystemSynchronizedRootGroup`(Share Extension)需 objectVersion≥70,被降 54 是格式不一致(Xcode 26 目前僅容忍)。**根治用 git clean filter**:`.gitattributes` 對 `ios/Runner.xcodeproj/project.pbxproj` 掛 `filter=pbxobjver`,git 端一律把 objectVersion 正規化回 70——工作區實體可被 flutter 改成 54(build 照吃無害),但 `git diff`/`status`/commit 永遠是 70、零 churn。**filter 是本機 git config 不隨 repo 走,各機需跑一次**:`git config filter.pbxobjver.clean "sed -E 's/objectVersion = [0-9]+;/objectVersion = 70;/'"`(裝好後 `git add --renormalize ios/Runner.xcodeproj/project.pbxproj` 生效)。
