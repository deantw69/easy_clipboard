...
- **原生接收(單一入口)**:`ios/Runner/DeepLinkChannel.swift`(`NSObject, FlutterPlugin`)。**FlutterPlugin 協定要求 `static func register(with registrar: FlutterPluginRegistrar)`**(不是 `FlutterPluginRegistry`,簽名不符會報 `does not conform to protocol 'FlutterPlugin'`);因 `addApplicationDelegate` 只收 `FlutterPlugin`,本類必須真的 conform。`AppDelegate.didInitializeImplicitFlutterEngine` 先 `pluginRegistry.registrar(forPlugin:"DeepLink")` 取 registrar 再 `DeepLinkChannel.register(with: registrar)`。
- **scene 架構(iOS 13+)收 URL 的關鍵(踩過雷)**:本 App 用 `FlutterSceneDelegate`。**只靠 `addApplicationDelegate` + `application(_:open:)` 收不到 scene 的 URL**(點 widget 完全無反應)——scene App 要 `registrar.addSceneDelegate(instance)` 且該類 conform `FlutterSceneLifeCycleDelegate`,實作 `scene(_:openURLContexts:)`(熱啟動)與 `@objc(scene:willConnectToSession:options:)`(冷啟動,URL 在 `connectionOptions.urlContexts`)才收得到。現同時註冊 application + scene delegate(application 當保險)。**不覆寫 SceneDelegate 子類**(會破壞分享流程且 override/super 有編譯風險,`FlutterSceneDelegate` 標頭沒宣告這些方法)。
- **冷啟動時序雷**:`channel` 在 `register` 就建立,但 Dart 端要到首幀後 `start()` 才 `setMethodCallHandler`,期間 `invokeMethod("route")` 會**遺失**(Dart 無 handler)。故用 `dartReady` 旗標:未就緒前 `handle` 一律存 `pending`,由 `getInitial` 取回並設 `dartReady=true`。
