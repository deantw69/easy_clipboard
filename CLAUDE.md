...
- iOS 主畫面 Widget 顯示釘選備忘錄(小尺寸)與最近幾筆清單(中/大尺寸)。**與鬧鐘 CountdownWidget(Live Activity,在 feat/alarm-tab)無關**,是獨立的 WidgetKit extension,屬共通功能在 **main** 開發。
...
- **資料契約**:`{ "pinned": Summary|null, "recent": [Summary...] }`,`Summary = { title, color(ARGB int?), todoCount, doneCount }`。`title` 取備忘錄本文首行,無則第一個待辦文字。recent 取 `visibleMemos` 前 5 筆。
